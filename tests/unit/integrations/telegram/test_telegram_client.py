...
class TestTelegramClient:
    ...
    @pytest.mark.asyncio
    async def test_start_polling(self, telegram_client):
        """Test starting the bot in polling mode."""
        # Mock the application
        mock_app = MagicMock()
        mock_app.initialize = AsyncMock()
        mock_app.start = AsyncMock()
        mock_app.updater.start_polling = AsyncMock()
        telegram_client.application = mock_app

        # Start polling
        await telegram_client.start_polling()
    ...
    @pytest.mark.asyncio
    async def test_start_polling_no_application(self, telegram_client):
        """Test starting the bot without an application."""
        # Ensure application is None
        telegram_client.application = None

        # Mock setup_command_handlers to set the application
        ...
        with patch.object(
            telegram_client, "setup_command_handlers", AsyncMock(side_effect=mock_setup)
        ) as mock_setup:
            # Start polling
            await telegram_client.start_polling()
            ...
            # Verify methods were called
            telegram_client.application.initialize.assert_called_once()
            telegram_client.application.start.assert_called_once()
            telegram_client.application.updater.start_polling.assert_called_once()

    @pytest.mark.asyncio
    async def test_stop_polling(self, telegram_client):
    ...
    @pytest.mark.asyncio
    async def test_stop_polling_no_application(self, telegram_client):
        """Test stopping the bot without an application."""
        # Ensure application is None
        telegram_client.application = None

        # Stop polling (should not raise an exception)
        await telegram_client.stop_polling()
